  ...
  def main(args: Array[String]): Unit = {
    ...
    var groupRdd1 = mapRdd.
      ...
      map(x=>{
        ...
        var summation = data.map(x=>{x.getDouble(qtyNo)}).sum
        var avg =1.0d
        if (size!=0){
          avg = summation/size
        }
    ...
    middleResult.createOrReplaceTempView("MIDDLETABLE")
        var finalResult = spark.sql("select regionid, product, yearweek, ratio from MIDDLETABLE")
        finalResult.show()

     }
//     var finalResult = spark.sql("select region")
